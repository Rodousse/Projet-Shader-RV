...
var unityObject = function() {
	...
	baseImageDomain = useSSL ? "https://ssl-webplayer.unity3d.com/" : "http://webplayer.unity3d.com/",
	baseUrl = baseDomain + "download_webplayer-3.x/",
	fullInstall = false,
	autoInstall = false,
	enableJava = true,
	...
	enableUnityAnalytics = !useSSL && false,
			...
			if (ua.win && ua.ie) {
				...
				if (av != null) {
					...
					for (var i = 0; i < av.length; ++i) {
						...
						if (compareVersions(rv, versionNumbers) <= 0) {
							return true;
						}
					}
				}
	...
	an = function() {
		...
		function sendUnityAnalytics(event, type, data, callback) {
			...
			if (callback) {
				...
				timer = setTimeout(img.onload,kAnalyticsTimeout);
		...
		function sendGoogleAnalytics(event, type, data, callback) {
			...
			if (callback) {
				...
				var callbackWrapper = function() {
					unityObject.googleAnalyticsCallback = null;
					clearTimeout(timer);
					callback();
				}
				
				unityObject.googleAnalyticsCallback = callbackWrapper;
				timer = setTimeout(callbackWrapper,kAnalyticsTimeout);
			}
	...
	function createMissingUnity(id, attributes, params, callback, broken, prevType, lastError) {
		...
		if (re) {
			var type = kStandard;
			if (!ua.x64 && enableJava && ua.java && !triedJava && !getCookie(triedJavaCookie)) {
				applets[id] = {
			...
			if (broken) {
				...
				var img = baseImageDomain + "/installation/getunityrestart.png";
				var imgWidth = 190;
				var imgHeight = 75;
			}
			else {
				var msg = "Unity Web Player. Install now!";
				var img = baseImageDomain + "installation/getunity.png";
				var imgWidth = 193;
				var imgHeight = 63;
			}
	...
	function getWinInstall() {
        ...
        if (ua.x64) {
            url = fullInstall ? "UnityWebPlayerFull64.exe" : "UnityWebPlayer64.exe";
        } else {
            url = fullInstall ? "UnityWebPlayerFull.exe" : "UnityWebPlayer.exe";
        }
	...
	function doJavaInstall(id) {
		...
		var p = {
			...
			image : baseImageDomain + "installation/unitylogo.png",
			centerimage : true,
			boxborder : false,
			scriptable : true,
			mayscript : true
	...
	return {
		...
		enableAnalytics : function(value) {
			enableUnityAnalytics = !useSSL && value;
		...
		// Set base domain url. Default is:
		// "http://webplayer.unity3d.com/
		setBaseDomain: function(value) {
			var oldDomain = baseDomain;
		    baseDomain = value;
			if (baseDomain[baseDomain.length-1] != "/"){
				baseDomain += "/";
			}
			baseUrl = baseDomain + baseUrl.substr(oldDomain.length);
			
		},
		
		// set base download url. Default is:
		// "http://webplayer.unity3d.com/download_webplayer-3.x/
		// this will also modify the baseDomain
